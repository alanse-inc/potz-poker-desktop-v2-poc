...
import { PlayerModal } from "../../setting/components/player_modal";
...
export function FirstGameSetting() {
  ...
  const handleBtnButtonClick = () => {
    ...
    // BTN 選択画面 (select-btn) に遷移
    navigate("/game/first-game/select-btn", {
      state: {
        smallBlind,
        bigBlind,
        minChip,
        ...
        >
          ...
          >
            <div className="flex h-full items-center gap-60 pl-16">
              <SettingBoard
                players={players}
                onClickSeat={(index) => setSelectedSeat(index)}
              />
              <GameSettingButtons
